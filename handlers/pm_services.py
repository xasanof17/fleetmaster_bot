...
- Show all PM rows
- Send entire Urgent/Oil list to each truck’s own group
"""
...
from services.group_map import get_group_id_for_unit        # ✅ your unit→group mapping
...
from keyboards.pm_services import (
    ...
    urgent_oil_list_keyboard,                                # ✅ NEW keyboard
)
...
async def urgent_list(cb: CallbackQuery):
    ...
    await cb.message.answer(
        text,
        reply_markup=urgent_oil_list_keyboard("urgent"),   # ✅ attach new keyboard
        parse_mode="Markdown"
    )
...
async def oil_change_list(cb: CallbackQuery):
    ...
    await cb.message.answer(
        text,
        reply_markup=urgent_oil_list_keyboard("oil"),      # ✅ attach new keyboard
        parse_mode="Markdown"
    )
...
async def send_list_to_groups(cb: CallbackQuery):
    """
    When the admin taps 📤 Send list to each group,
    send each truck line to its own mapped group.
    """
    list_type = cb.data.split(":")[1]
    await cb.answer("📡 Sending to groups…")

    rows = (
        await google_pm_service.get_urgent_list() if list_type == "urgent"
    ...
    for r in rows:
        ...
        if not group_id:
            # ❌ reply ONLY in admin’s chat, not as broadcast
            await cb.message.answer(f"❌ No group mapped for truck {truck}. Please set it up.")
            skipped += 1
            continue
